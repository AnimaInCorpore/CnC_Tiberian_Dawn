...
- Finish Win95 main menu/game selection parity: new game/campaign progression and any remaining internet/WChat handoff behavior (`Select_Game()`, `Main_Menu()`) (`src/port_runtime.cpp`, `src/menus.cpp`).
...
- Verify `.BIN` map loads across multiple theaters now that `TemplateType` is 8-bit again (ensure tile icons and clear terrain resolve correctly) (`src/map.cpp`, `src/include/legacy/defines.h`).
