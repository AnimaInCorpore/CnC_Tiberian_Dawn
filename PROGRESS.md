...
| `TITLE/PALETTE` | `src/load_title.cpp` | manual | Fixed PCX palette handling for Win95 parity: avoid double-downshifting already-6-bit palettes, honor the optional 0x0C palette marker, and respect `bytes_per_line` padding so RLE decode doesn’t desync (prevents overly-dark/garbled title/menu UI). |
