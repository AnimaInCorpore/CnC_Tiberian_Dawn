...
| --- | --- | --- | --- |
| Parity checks (aadata..bdata) | `src/aadata.cpp`, `src/abstract.cpp`, `src/adata.cpp`, `src/aircraft.cpp`, `src/alloc.cpp`, `src/anim.cpp`, `src/audio.cpp`, `src/base.cpp`, `src/bbdata.cpp`, `src/bdata.cpp` | build | Verified the next 10 sources against their legacy counterparts; restored `AircraftClass::Debug_Dump` box-drawing output using exact CP437 bytes (avoids signed-`char` hex-escape issues) and kept the `Exit_Object` signature aligned with the `TechnoClass` virtual. |
...
| Parity checks (stats + core command helpers) | `src/stats.cpp`, `src/tarcom.cpp`, `src/target.cpp`, `src/team.cpp`, `src/teamtype.cpp`, `src/techno.cpp`, `src/template.cpp`, `src/super.cpp`, `src/tdata.cpp`, `src/tcpip.cpp` | build | Restored Win95 `Send_Statistics_Packet()` packet assembly + deferred-send behavior using `PacketClass` and the existing UDP-backed `Send_Data_To_DDE_Server(...)` path; reviewed the next 9 units for behavioral parity (only include-path hygiene, formatting, and safe `snprintf` changes). |
| Parity checks (terrain/theme/toggle/trigger/turret/UI helpers) | `src/terrain.cpp`, `src/theme.cpp`, `src/toggle.cpp`, `src/trigger.cpp`, `src/turret.cpp`, `src/txtlabel.cpp`, `src/udata.cpp`, `src/utracker.cpp`, `src/vector.cpp`, `src/visudlg.cpp` | build | Verified the next 10 sources against their legacy counterparts; restored `src/visudlg.cpp` to match Win95 dialog flow (Init geometry, caption draw, input loop + keyboard navigation, reset applies Options values), and tightened include-path hygiene + safe `snprintf` conversions in a few INI/asset-name call sites. |
...
| Parity fix (score screen palette signedness) | `src/score.cpp`, `src/include/legacy/score.h` | build | Fixed a cross-platform parity hazard where score-screen font palettes contain indices > `0x7F` but were typed as `char` (signed on some platforms); palette pointers now use `void const*` and the local score palettes use `unsigned char` so palette indices are stable. |
...
| Include path hygiene (UI/helpers batch) | `src/shapebtn.cpp`, `src/object.cpp`, `src/scroll.cpp`, `src/audio_play.cpp`, `src/interpal_asm.cpp`, `src/ending.cpp`, `src/dial8.cpp`, `src/door.cpp`, `src/dpmi.cpp`, `src/cheklist.cpp`, `src/descdlg.cpp`, `src/msglist.cpp`, `src/bitblt_helpers.cpp`, `src/mousehelp.cpp`, `src/control.cpp`, `src/layer.cpp`, `src/list.cpp`, `src/debug.cpp`, `src/tab.cpp`, `src/colrlist.cpp`, `src/goptions.cpp`, `src/credits.cpp`, `src/edit.cpp`, `src/radio.cpp`, `src/slider.cpp`, `src/interpal.cpp`, `src/gauge.cpp`, `src/checkbox.cpp`, `src/textbtn.cpp`, `src/include/abstract.h` | build | Switched remaining translation units still including `function.h` to `legacy/function.h` to avoid case-sensitive header resolution ambiguity and keep builds deterministic across platforms. |
...
| `MOVIE/VQA` (audio) | `src/movie.cpp`, `src/vqa_decoder.cpp`, `src/audio_play.cpp` | differs | Added initial VQA audio playback support by extracting `SND0` PCM payloads and mixing them through the existing SDL mixer so movie audio shares the same output device/volume semantics as other sounds. |
...
| `TITLE/PCX` | `src/load_title.cpp` | manual | Hardened PCX RLE decoding by rejecting zero-length runs and failing when the stream does not fill the expected pixel buffer; also simplified `Apply_Title_Palette` allocation/assignment. Matched Win95 `Read_PCX_File` behavior when `bytes_per_line` differs from width by consuming per-scanline padding bytes instead of treating them as pixels (prevents corrupt title/menu backgrounds on padded PCXs). |
