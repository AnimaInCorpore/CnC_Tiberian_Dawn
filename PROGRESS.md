...
| `SIDEBAR.CPP` (clock translucent table) | `src/sidebar.cpp` | Stopped forcing `_RETRIEVE` so the sidebar translucent-table cache (`TCLOCK.MRF` etc) is generated and written on demand instead of failing when the `.MRF` files aren’t shipped with extracted CD data. |
