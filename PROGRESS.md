...
| `TITLE/PCX` | `src/load_title.cpp` | manual | Hardened PCX RLE decoding by rejecting zero-length runs and failing when the stream does not fill the expected pixel buffer; also simplified `Apply_Title_Palette` allocation/assignment. |
