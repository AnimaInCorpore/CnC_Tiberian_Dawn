...
void MapClass::Logic(void)
{
	...
	*/
	int subcount = 30;
	int index = TiberiumScan;
	for (; index < MAP_CELL_TOTAL; index++) {
		CELL cell = index;
...
if (BlubCell->Overlapper[1]) {
	...
	for (cell = 0; cell < MAP_CELL_TOTAL; cell++) {
		...
		if (obj) {
			if (
				((reinterpret_cast<std::uintptr_t>(obj) & 0xff000000u) != 0) ||
				((reinterpret_cast<std::uintptr_t>(obj->Next) & 0xff000000u) != 0) ||
				((reinterpret_cast<std::uintptr_t>(obj->Trigger) & 0xff000000u) != 0) ||
				obj->IsInLimbo ||
				(static_cast<unsigned int>(Coord_Cell(obj->Coord)) > 4095u)
				)
				return (false);
		}
