...
/* $Header:   F:\projects\c&c\vcs\code\field.cpv   1.0   22 Apr 1996 08:21:58   PWG  $ */
/***************************************************************************
 *                                                                         *
...
FieldClass::FieldClass(char *id, char *data)
{
	...
	Size		= static_cast<unsigned short>(std::strlen(data) + 1);
	Data		= new char[Size];
	std::memcpy(Data, data, Size);
	Next		= nullptr;
}
...
void FieldClass::Host_To_Net(void)
{
	...
	switch (DataType) {
		...
		case TYPE_UNSIGNED_SHORT:
			*reinterpret_cast<unsigned short *>(Data) = htons(*reinterpret_cast<unsigned short *>(Data));
			break;

		case TYPE_LONG:
		case TYPE_UNSIGNED_LONG:
			*reinterpret_cast<unsigned long *>(Data) = htonl(*reinterpret_cast<unsigned long *>(Data));
			break;
...
void FieldClass::Net_To_Host(void)
{
	...
	switch (DataType) {
		...
		case TYPE_SHORT:
		case TYPE_UNSIGNED_SHORT:
			*reinterpret_cast<unsigned short *>(Data) = ntohs(*reinterpret_cast<unsigned short *>(Data));
			break;

		case TYPE_LONG:
		case TYPE_UNSIGNED_LONG:
			*reinterpret_cast<unsigned long *>(Data) = ntohl(*reinterpret_cast<unsigned long *>(Data));
			break;
