...
void Explosion_Damage(COORDINATE coord, unsigned strength, TechnoClass* source, WarheadType warhead) {
    ...
    if ((unsigned)cell >= (unsigned)MAP_CELL_TOTAL) return;

    CellClass* cellptr = &Map[cell];
    ObjectClass* impacto = cellptr->Cell_Occupier();

    count = 0;
    for (int face = static_cast<int>(FACING_NONE); face < static_cast<int>(FACING_COUNT); ++face) {
        FacingType i = static_cast<FacingType>(face);
    ...
    for (int index = 0; index < count; ++index) {
        ...
        if (object->IsDown && !object->IsInLimbo && distance < range) {
            int damage = static_cast<int>(strength);
    ...
    cellptr = &Map[cell];
    cellptr->Reduce_Tiberium(static_cast<int>(strength / 10));
    if (cellptr->Overlay != OVERLAY_NONE) {
        ...
        if (optr->IsWall) {
            if (whead->IsWallDestroyer || (whead->IsWoodDestroyer && optr->IsWooden)) {
                Map[cell].Reduce_Wall(static_cast<int>(strength));
            }
        }
    }
}
