...
void CellClass::Overlap_Down(ObjectClass * object)
{
	...
	if (!object) return;
	for (index = 0; index < static_cast<int>(sizeof(Overlapper)/sizeof(Overlapper[0])); index++) {
		...
		for (index = 0; index < static_cast<int>(sizeof(Overlapper)/sizeof(Overlapper[0])); index++) {
			switch (Overlapper[index]->What_Am_I()) {
				...
				default:
					...
					index = static_cast<int>(sizeof(Overlapper)/sizeof(Overlapper[0]));
					break;
			}
		}
	}
	...
	//			 with shadow.
			...
			*/
			if (ttype->Get_Image_Data()) {
				...
				if (remap) {
										LogicPage->Remap(x+Map.TacPixelX, y+Map.TacPixelY, ICON_PIXEL_W, ICON_PIXEL_H, static_cast<unsigned char const*>(remap));
				}
			}
		...
		if (steel) {
			...
			if (GameToPlay != GAME_NORMAL && (Random_Pick(1, 2) == 1 || !object->House->BScan)) {
				...
				*/
				...
				while (what == -1) {
										what = _what[Random_Pick(0u, static_cast<unsigned>(sizeof(_what)/sizeof(_what[0]) - 1))];
					...
					for (index = 0; index < 5; index++) {
						static InfantryType _inf[] = {
							...
							INFANTRY_RAMBO
												...
												InfantryTypeClass::As_Reference(_inf[Random_Pick(0u, static_cast<unsigned>(sizeof(_inf)/sizeof(_inf[0]) - 1))]).Create_And_Place(Cell_Number(), object->Owner());
					}
					return(false);
