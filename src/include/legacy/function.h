...
// Standard C headers used throughout the legacy runtime.
#include <cassert>
#include <cstdarg>
#include <cstddef>
#include <cctype>
...
// Networking/UI maintenance headers are omitted for the initial modern port.
// They will be reinstated once those subsystems are migrated.
//#include "base.h"
//#include "ipxmgr.h"
//#include	"combuf.h"
//#include	"connect.h"
...
void const * Hires_Retrieve(char const *name);
...
int Sim_Random(void);
inline int Random() { return Sim_Random(); }
...
*/
#define	XYP_COORD(x,y)	(((x)*ICON_LEPTON_W)/CELL_PIXEL_W + ((((unsigned)((((y)*ICON_LEPTON_H)/CELL_PIXEL_H) & 0xFFFF)))<<16))
