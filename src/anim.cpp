...
void AnimClass::Init(void)
{
	...
	ptr = new AnimClass();
	VTable = reinterpret_cast<void**>(reinterpret_cast<char*>(ptr) + sizeof(AbstractClass) - sizeof(void*))[0];
	delete ptr;
}
	...
	Class(&AnimTypeClass::As_Reference(animnum))
{
	...
	if (Class->Stages == -1) {
		const_cast<int&>(Class->Stages) = Get_Build_Frame_Count(Class->Get_Image_Data());
	}
	if (Class->LoopEnd == -1) {
		const_cast<int&>(Class->LoopEnd) = Class->Stages;
	}
...
//						AnimTypeClass const * aptr = &AnimTypeClass::As_Reference(Class->ChainTo);
						...
						const_cast<AnimTypeClass const*&>(Class) = &AnimTypeClass::As_Reference(Class->ChainTo);
...
void AnimClass::Code_Pointers(void)
{
	...
	------------------------------ Code 'Class' ------------------------------
	*/
	const_cast<AnimTypeClass const*&>(Class) = reinterpret_cast<AnimTypeClass*>(static_cast<std::uintptr_t>(Class->Type));

	/*
	----------------------------- Code 'Object' ------------------------------
	*/
	if (Object) {
		Object = reinterpret_cast<ObjectClass*>(static_cast<std::uintptr_t>(Object->As_Target()));
	}
...
void AnimClass::Decode_Pointers(void)
{
	...
	----------------------------- Decode 'Class' -----------------------------
	*/
	const auto class_code = static_cast<AnimType>(reinterpret_cast<std::uintptr_t>(Class));
	const_cast<AnimTypeClass const*&>(Class) = &AnimTypeClass::As_Reference(class_code);
	...
	*/
	if (Object) {
		Object = As_Object(static_cast<TARGET>(reinterpret_cast<std::uintptr_t>(Object)));
