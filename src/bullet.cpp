...
*/
void* BulletClass::VTable = nullptr;
...
//			IsLocked = true;
			...
			*/
			...
			if (!forced && (Class->IsDropping || !Fuse_Checkup(coord))) {
				...
				*/
				Mark();
				COORDINATE fuse_target = Fuse_Target();
...
//		Timer++;
	...
	*/
	if (Altitude) {
		CC_Draw_Shape(shapeptr, shapenum, x, y, window, SHAPE_PREDATOR|SHAPE_CENTER|SHAPE_WIN_REL|SHAPE_FADING, nullptr, Map.FadingShade);
	...
	*/
	...
	CC_Draw_Shape(shapeptr, shapenum, x, y, window, flags|SHAPE_CENTER|SHAPE_WIN_REL, nullptr, Map.UnitShadow);
}
...
bool BulletClass::Unlimbo(COORDINATE coord, DirType dir)
{
	...
	*/
	if (ObjectClass::Unlimbo(coord)) {
		...
		**	Arm the fuse.
		*/
		Arm_Fuse(Coord, tcoord, range, (As_Aircraft(TarCom) ? 0 : Class->Arming));
		...
		*/
		...
		if (Class->IsArcing) {
			Altitude = 1;
			Riser = ((Distance(tcoord)/2) / (speed+1))*kGravity;
			Riser = MAX(Riser, 10);
		}
		if (Class->IsDropping) {
