...
///////////////////////#endif	//(0)
	...
	*/
	if (text && *text) {
		...
		if (LogicPage && (unsigned)x < SeenBuff.Get_Width() && (unsigned)y < SeenBuff.Get_Height()) {
...
static void Dialog_VSPrintf(char* buffer, size_t buffer_size, char const* format, va_list args)
{
	...
	*/
	if (text != TXT_NONE) {
		...
		*/
		...
		Dialog_VSPrintf(buffer, sizeof(buffer), tptr, arg);
		va_end(arg);
...
void Fancy_Text_Print(char const * text, unsigned x, unsigned y, unsigned fore, unsigned back, TextPrintType flag, ...)
{
	...
	*/
	if (text) {
		...
		*/
		va_start(arg, flag);
		Dialog_VSPrintf(buffer, sizeof(buffer), text, arg);
		va_end(arg);
