...
AircraftType AircraftTypeClass::From_Name(char const *name)
{
	if (name) {
		for (AircraftType classid = AIRCRAFT_FIRST; classid < AIRCRAFT_COUNT; classid++) {
			if (strcasecmp(Pointers[classid]->IniName, name) == 0) {
				return(classid);
			}
		}
	}
	...
	for (index = AIRCRAFT_FIRST; index < AIRCRAFT_COUNT; index++) {
		...
		*/
		...
		if ( Get_Resolution_Factor() ) {
			base_name = std::string(uclass.IniName) + "ICNH";
		} else {
			base_name = std::string(uclass.IniName) + "ICON";
		}
		std::string fullname = base_name + ".SHP";
		((void const *&)uclass.CameoData) = MixFileClass::Retrieve(fullname.c_str());

		/*
		**	Generic shape for all houses load method.
		*/
		fullname = std::string(uclass.IniName) + ".SHP";
	...
	if (theater != LastTheater){
		if ( Get_Resolution_Factor() ) {
			...
			for (index = AIRCRAFT_FIRST; index < AIRCRAFT_COUNT; index++) {
				...
				std::string buffer_str(uclass.IniName, 4); // "%.4s" from sprintf
				buffer_str += "ICNH";
				
				std::string fullname_str = buffer_str + Theaters[theater].Suffix;
				cameo_ptr = MixFileClass::Retrieve(fullname_str.c_str());
				if (cameo_ptr){
