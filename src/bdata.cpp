	...
	for (StructType sindex = STRUCT_FIRST; sindex < STRUCT_COUNT; sindex++) {
		...
		*/
		if (building.IsBuildable) {
			std::string cameo_name = building.IniName;
			cameo_name += Get_Resolution_Factor() ? "ICNH" : "ICON";
			std::string cameo_fullname = cameo_name + ".SHP";
			((void const *&)building.CameoData) = MixFileClass::Retrieve(cameo_fullname.c_str());
		}

		/*
		**	Fetch the construction animation for this building.
		*/
		std::string buildup_name = std::string(building.IniName) + "MAKE.SHP";
		void const * dataptr = MixFileClass::Retrieve(buildup_name.c_str());
		...
		*/
		std::string shape_name = std::string(building.IniName) + ".SHP";
		((void const *&)building.ImageData) = MixFileClass::Retrieve(shape_name.c_str());
	}
...
StructType BuildingTypeClass::From_Name(char const *name)
{
	if (name) {
		for (StructType classid = STRUCT_FIRST; classid < STRUCT_COUNT; classid++) {
			if (strcasecmp(As_Reference(classid).IniName, name) == 0) {
				return(classid);
			}
		}
	}
 ...
 *    there are no suitable buildings, then nullptr is returned. Typical use of this function is  *
	...
	if (theater != LastTheater){
		for (StructType sindex = STRUCT_FIRST; sindex < STRUCT_COUNT; sindex++) {
			...
			if (classptr->IsTheater) {
				std::string filename = std::string(classptr->IniName) + Theaters[theater].Suffix;
				((void const *&)classptr->ImageData) = MixFileClass::Retrieve(filename.c_str());
			}

			if ( Get_Resolution_Factor() ) {
				((void const *&)classptr->CameoData) = nullptr;

				std::string cameo_name = std::string(classptr->IniName, 4) + "ICNH";
				std::string cameo_fullname = cameo_name + Theaters[theater].Suffix;
				void const * cameo_ptr = MixFileClass::Retrieve(cameo_fullname.c_str());
				if (cameo_ptr){
