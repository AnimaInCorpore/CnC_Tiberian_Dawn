...
extern bool Force_CD_Available(int cd);
extern void __cdecl Prog_End(void);
...
bool Mix_Offset(char const* filename, void** realptr, MixFileClass** mixfile, long* offset, long* size) {
	if (!filename) return false;
	if (MixFileClass::Offset(filename, realptr, mixfile, offset, size)) {
		return true;
	}
	char const* base = Basename(filename);
	if (base && base != filename) {
		return MixFileClass::Offset(base, realptr, mixfile, offset, size);
	}
	return false;
}
...
long CCFileClass::Read(void* buffer, long size)
{
	...
	*/
	if (Pointer) {
		...
		if (size) {
			std::memmove(buffer, static_cast<unsigned char*>(Pointer) + Position, static_cast<std::size_t>(size));
			Position += size;
		}
		if (opened) Close();
		return(size);
...
{
	if (Mix_Offset(File_Name(), nullptr, nullptr, nullptr, nullptr)) {
		return(true);
	}
 ...
 * CCFileClass::Close -- Closes the file.                                                      *
 *                                                                                             *
 *    Closes the file. This is necessary because if the file is part of the mixfile system     *
 *    then special record keeping needs to be maintained.                                      *
 *                                                                                             *
 * INPUT:   none                                                                               *
 *                                                                                             *
 * OUTPUT:  none                                                                               *
...
int CCFileClass::Open(int rights)
{
	...
	*/
	...
	if (Mix_Offset(File_Name(), &Pointer, &mixfile, &Start, &Length)) {
		...
		*/
		if (!Pointer) {
			...
			*/
			...
			CDFileClass::Open(mixfile->Filename, READ);
