...
void Decode_All_Pointers(void)
{
	...
	**	PlayerPtr.
	*/
	PlayerPtr = HouseClass::As_Pointer(static_cast<HousesType>(reinterpret_cast<std::uintptr_t>(PlayerPtr)));
	...
	*/
	for (i = 0; i < CurrentObject.Count(); i++) {
		CurrentObject[i] = As_Object(static_cast<TARGET>(reinterpret_cast<std::uintptr_t>(CurrentObject[i])));
