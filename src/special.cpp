...
void Special_Dialog(void) {
  ...
  static struct {
    ...
    CheckBoxClass* Button;
      ...
      {TXT_SEPARATE_HELIPAD, 0, 0}, {TXT_VISIBLE_TARGET, 0, 0}, {TXT_TREE_TARGET, 0, 0},
      {TXT_MCV_DEPLOY, 0, 0},       {TXT_SMART_DEFENCE, 0, 0}, {TXT_THREE_POINT, 0, 0},
  ...
  TextButtonClass cancel(201, TXT_CANCEL, TPF_6PT_GRAD | TPF_NOSHADOW,
                         OPTION_X + OPTION_WIDTH - 50, OPTION_Y + OPTION_HEIGHT - 15);
  buttons = &ok;
  cancel.Add(*buttons);
  ...
  while (process) {
    ...
    */
    ...
    if (display) {
      ...
      for (int index = 0; index < static_cast<int>(sizeof(_options) / sizeof(_options[0])); index++) {
        Fancy_Text_Print(_options[index].Description, _options[index].Button->X + 10,
  ...
  /*
  ** Now apply the new values (if any).
  */
  Special.IsSeparate = oldspecial.IsSeparate;
  Special.IsNamed = oldspecial.IsNamed;
  Special.IsDefenderAdvantage = oldspecial.IsDefenderAdvantage;
  Special.IsVisibleTarget = oldspecial.IsVisibleTarget;
  Special.IsTreeTarget = oldspecial.IsTreeTarget;
  Special.IsMCVDeploy = oldspecial.IsMCVDeploy;
  Special.IsSmartDefense = oldspecial.IsSmartDefense;
  Special.IsThreePoint = oldspecial.IsThreePoint;
  Special.IsTGrowth = oldspecial.IsTGrowth;
  Special.IsTSpread = oldspecial.IsTSpread;
  Special.IsTFast = oldspecial.IsTFast;
