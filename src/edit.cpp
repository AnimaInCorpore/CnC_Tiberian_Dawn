 ...
 *                                                                                             *
 *          x,y   -- The pixel coordinate of the upper left corner of the edit box.            *
 *                                                                                             *
...
void EditClass::Draw_Text(char const * text)
{
	if (FontPtr == GradFont6Ptr) {
		...
		Conquer_Clip_Text_Print(text, X+1, Y+1, Color, TBLACK, TextFlags | flags, Width-2, 0);

		if (Has_Focus() && std::strlen(text) < (unsigned)MaxLength &&
				(String_Pixel_Width(text) + String_Pixel_Width ("_") < Width-2) ) {
			Conquer_Clip_Text_Print( "_", X+1+String_Pixel_Width(text), Y+1, Color, TBLACK, TextFlags | flags, Width-2, 0);
		}
	} else {
		Conquer_Clip_Text_Print(text, X+1, Y+1, Has_Focus() ? BLUE : WHITE, TBLACK, TextFlags, Width-2, 0);

		if (Has_Focus() && std::strlen(text) < (unsigned)MaxLength &&
				(String_Pixel_Width(text) + String_Pixel_Width ("_") < Width-2) ) {
			Conquer_Clip_Text_Print("_",X+1+String_Pixel_Width(text),Y+1,BLUE,TBLACK, TextFlags, Width-2, 0);
		}
	}
	...
	switch (ascii) {
		...
		*/
		default:
			...
			*/
			if ((String_Pixel_Width(String) + Char_Pixel_Width((char)ascii) ) >= (Width-2)) {
				break;
			}
			...
			*/
			if (!std::isgraph(ascii) && ascii != ' ') break;
			...
			*/
			if ((EditFlags & UPPERCASE) && std::isalpha(ascii)) {
				ascii = (KeyASCIIType)std::toupper(ascii);
			}

			if ((!(EditFlags & NUMERIC) || !std::isdigit(ascii)) &&
				(!(EditFlags & ALPHA) || !std::isalpha(ascii)) &&
				(!(EditFlags & MISC) || std::isalnum(ascii)) &&
				ascii != ' ') {
					break;
			}
